...
describe('basic tests', () => {
    ...
    it('Can map by key', async () => {
        ...
        const event3 = mkEvent('Event3',"This is an string event with effect '12'")
        const probe = Among_.fromKind<MyEvents>(event3)
        const expected_ = 'THIS'
        //act
        const action = probe.mapByKey('Event3', value => {
            const THIS = value.split(' ')[0].toUpperCase()
            return THIS
        })
        //check
        const actual = action.unsafeRun()[1]
        expect(actual).toStrictEqual(expected_)
    ...
    it('Can select a kind of avent among others', () => {
        //prepare
        let c = 12 as const
        const expected = `This is an string event with effect '${c}'` as const
        type MyEvents = {
        ...
        const probe =  Among_.fromKind<MyEvents>(event3)
        //act
        const action = probe._select('Event3')
        //check
        const actual = action.map( a => a.unsafeRun()).unsafeRun()
        const expected_ = { hasError: false , value: ['Event3', expected]}
        expect(actual).toEqual(expected_)
